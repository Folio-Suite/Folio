...
| ResearchKit | No additional implementation libraries yet | Existing Research shell |
...
Write's application still owns NSDocument and its windows. The native package schema, document identifier, pasteboard identifier, supported formatting, and current development packaging arrangement are unchanged in this pass. A separately named persistence library, Swift-backed operations, namespace adoption, and the broader shell acceptance requirements remain subsequent work under #14.
