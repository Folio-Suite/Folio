...
Write’s domain host owns the authoritative Work Session and accepts requests from multiple Suite applications. Research and Composer govern Source Library and Arrangement operations through their respective Kits and hosts; Arrangements include Editions under [ADR 0011](../adr/0011-composer-arrangements-and-editions.md). Sharing framework binaries does not create shared runtime state. Host packaging and XPC transport remain unselected. Applications own presentation, selections, navigation, windows, and unfinished interaction state; they share such state with the Session only as needed to safeguard data. Explicit reveal/navigation actions may coordinate presentation.
