...
Folio is a monorepo for a large integrated Suite. Retain the existing workspace and place substantive implementation in the domain Kits and their named internal modules. Continue with Objective-C, AppKit storyboards, and visually authored Core Data models; introduce Swift selectively when it provides a concrete benefit. The first usable result must preserve the familiar macOS document experience and authored meaning while establishing module ownership that will survive later growth.

## Solution

Build the first Cocoa-first Folio shell by evolving the existing workspace and domain frameworks. Repository rebuilding is not a prerequisite. Write provides a modest but usable native document editor: create a Work, enter text, apply basic semantic or explicit formatting, undo and redo, and save and reopen a Core Data-backed package. Research remains a runnable domain shell, without requiring a research workflow to make Write useful. Composer's existing provisional shell remains part of the coordinated Suite build; full Arrangement or Edition behavior is separate work.

Objective-C supplies application/document plumbing, AppKit integration, public Cocoa-facing interfaces, and the Core Data adapter. Swift may implement bounded operations when useful, but introducing a language crossing is not an acceptance requirement. Any crossing introduced must remain narrow and be exercised by real application behavior.

FolioKit provides shared foundations. WriteKit, ResearchKit, and ComposerKit provide their respective public domain interfaces and may include reusable UI. Named internal modules compile directly into their owning Kits; applications do not bypass the public interfaces. FolioKit, Write, Research, and Composer share one repository and coordinated revision history while retaining separate Xcode projects and explicit module ownership.
...
32. As a developer, I want internal modules to own actual implementation within their Kits, so that module ownership does not become an empty organizational promise.
33. As a developer, I want Swift introduced only for useful operations through narrow Cocoa-facing interfaces, so that language choices do not require duplicating the semantic model.
...
- Continue from the implemented Kit consolidation and editor. Finish the remaining shell behaviors through the existing public interfaces; no repository cleanup, skeleton rebuild, or restoration of former library targets is required.
- Preserve the monorepo and existing workspace. FolioKit, Write, Research, and Composer retain explicit ownership of their Xcode projects, framework targets, resources, and tests. Commit coordinated source, project, scheme, and documentation changes together.
- Use `dev.foliosuite` as the primary namespace for application, framework, document-type, and other newly defined identifiers. Preserve the FK, FW, FR, and FC Objective-C naming conventions for their respective domains; Swift names need not duplicate Objective-C prefixes merely for symmetry.
...
- Deliver Write's small editor and Research's runnable application/framework shell, preserving Composer's provisional shell in Suite validation. Do not infer a complete Research workflow or Composer production model from existing application targets.

### Frameworks and internal modules

Implementation sources compile directly into their owning frameworks under [ADR 0009](../adr/0009-continue-cocoa-suite-with-domain-kits.md). Named internal modules retain their responsibilities; separate libraries require demonstrated reuse and are not required by this milestone. See the [current layout](../architecture/current-library-layout.md).
...
| FKXMLSupport | Future XML exchange support | Retain the named internal module; no XML native persistence or speculative exporter is required |
...
| FWEditor | AppKit text adaptation, semantic formatting commands, selection, and undo integration | Compiled directly into WriteKit; no separate library required |
| WriteKit private persistence adapter | Write-specific Core Data schema, context/store ownership, snapshot conversion, and native persistence adapter | FWWorkStore currently lives within FWManuscript; keep persistence private and separable without requiring the former proposed FWPersistence target |
...
| ComposerKit | Public Arrangement domain framework, including Editions | Preserve the current provisional shell; production storage and publication are separate work |

- The normal source and explicitly configured dependency direction is application → its domain framework → FolioKit. Keep the implementation graph acyclic. Internal module names describe ownership within a Kit, not additional public import surfaces.
- An application or another domain does not reach through a framework to consume its private implementation. Reusable model and UI declarations are exposed deliberately through the owning framework, with one authoritative implementation.
- Frameworks own the public interfaces, enumerated in explicit Clang module maps, umbrella headers, and Xcode public-header membership. Keep headers beside their implementation files, with one source for each declaration. A new framework is not required merely because an implementation changes language. Any later extracted library remains private to its owning Kit unless deliberately adopted as a separate public interface.
...
- Swift is optional and must earn its place through a concrete implementation benefit. No demonstration method or arbitrary language quota is required. If a Swift-backed operation is introduced, exercise it through the same public interface used by the real application.
...
- Represent a Work that owns authored Content Units separately from its Manuscript's ordered placement of them. New begins with one editable Content Unit and its placement. The accepted flat Manuscript sidebar adds, renames, selects, and reorders text Content Units; nested structure, deletion, and unplaced-content UI remain deferred.
...
- The primary formatting controls live in a native window toolbar defined in the WriteKit storyboard. Semantic E controls apply emphasis and strong emphasis. The accepted Appearance popover exposes independent Bold, Italic, Underline, and Strikethrough, with menu actions and shortcuts. Tooltips and help explain meaning versus presentation. Preserve the implemented overlap warning, explicit conversion, and undoable dismissal behavior described in the [editor contract](../architecture/native-work-v1.md#presentation-and-emphasis); presentation must not silently become meaning.
...
- Carry the prototype's semantic specialization probe into the Kit implementation: a separately defined specialization with an additional capability and persisted field must reconstruct with its identity and meaning intact. Conflicting definitions are rejected. This remains an interface/serialization proof independent of implementation language, not a plugin loader.
...
- NSDocument initially supplies the application lifecycle. Write's persistence implementation is isolated in its private adapter behind WriteKit and hosted in the Write process for this milestone. A later domain-owned executable remains a hosting option requiring lifecycle proof, not part of this shell.
...
- The amended ADR 0004 and Work Session contract place authority in document-domain hosts, superseding a mandatory central menu bar helper. Write owns Works, Research owns Source Libraries, and Composer owns Arrangements, including Editions under ADR 0011. This shell implements only the bounded in-process NSDocument stage. Loading a Kit in several processes does not establish shared authority.
- ADR 0009 supersedes mandatory mixed-language implementation and separate internal-library targets. ADR 0010 defines document-wide Undo and durable history; this shell's live Undo does not establish durable history. ADR 0011 defines Composer ownership and explicit archival associations; the provisional Composer document does not implement that contract.
...
- If Swift-backed operations are introduced, exercise actual callers across the language seam and verify equivalent semantic behavior and propagated errors. Do not introduce Swift solely to satisfy a test, or expose implementation-only dependency types to application callers.
- Verify a clean workspace build of Write, Research, Composer, and the domain frameworks. Check for cycles and private-interface bypasses. Validate published headers/module maps and an isolated outside consumer using the Kit-interface checks. Inspect generated interfaces and linked products where needed to distinguish explicit dependencies from compiler-emitted transitive dependencies.
...
- Nested or unplaced-content authoring UI, Content Unit deletion, Layout/composition, Arrangements and Editions, Renditions, publication/export, full semantic vocabularies, or complete Profile/Theme support. The accepted flat Manuscript sidebar remains in scope.
