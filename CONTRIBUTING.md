...
For the editor and package tests, select **Write** in Xcode and use Product → Test. That scheme includes FolioKitTests, WriteKitTests, WriteTests, and WriteUITests. Start native UI runners through Xcode, or prepare the signed test products with build-for-testing before using the CLI test runner. The shared **Folio** scheme also covers the Research shell, whose tests remain templates. Passing the current tests does not prove cross-application Work Session or archival behavior.

## Names and ownership

The current Objective-C implementation retains `net.ctwelve.Folio` bundle and type identifiers; the first-shell specification plans adoption of `dev.foliosuite`. Objective-C prefixes are **FK** for FolioKit, **FW** for Write, and **FR** for Research. The current document subclasses follow those prefixes; other generated application classes may acquire prefixes when developed.

Write's first editor uses `.folio-work` packages containing a Core Data store, with NSDocument hosting in-process persistence behind WriteKit. Research retains its empty `.folio-research-prototype` SQLite template. The early Write package schema is documented in `Write/docs/native-work-v1.md`; neither application implements complete archival folio exchange. Separately hosted domain persistence and shared editing still require lifecycle design and proof; the first editor does not settle issue #13 or implement the older helper-owned Work Session contract.
