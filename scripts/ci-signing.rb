...
keychain = File.join(root, 'folio-ci.keychain-db')
certificate = File.join(root, 'folio-ci.p12')
search_list = File.join(root, 'folio-original-keychains.json')
...
when 'import'
  ...
  begin
    ...
    File.open(ENV.fetch('GITHUB_ENV'), 'a') { |file| file.puts("FOLIO_SIGNING_IDENTITY=#{identities.first}") }
...
when 'cleanup'
  ...
  abort 'Usage: ruby scripts/ci-signing.rb import|cleanup'
end
